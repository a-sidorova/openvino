...
std::shared_ptr<ov::Model> MatMulFunction::initOriginal() const {
    ...
    if (precisions[1] != ov::element::f32) {
        matmul = std::make_shared<op::TypeRelaxed<op::v0::MatMul>>(
                ...
                ngraph::op::TemporaryReplaceOutputType(data0, element::f32).get(),
                ngraph::op::TemporaryReplaceOutputType(data1, element::f32).get());
    } else {
...
std::shared_ptr<ov::Model> MatMulFunction::initReference() const {
    ...
    if (precisions[1] != ov::element::f32) {
        matmul = std::make_shared<op::TypeRelaxed<op::v0::MatMul>>(
                std::vector<element::Type>{element::f32, element::f32},
                std::vector<element::Type>{ element::f32 },
                ngraph::op::TemporaryReplaceOutputType(indata0, element::f32).get(),
                ngraph::op::TemporaryReplaceOutputType(indata1, element::f32).get());
    } else {
        ...
        case 0: {
            ...
            if (precisions[1] != ov::element::f32) {
                result = std::make_shared<op::TypeRelaxed<op::v0::MatMul>>(
                         std::vector<element::Type>{element::f32, element::f32},
                         std::vector<element::Type>{ element::f32 },
                         ngraph::op::TemporaryReplaceOutputType(transpose, element::f32).get(),
                         ngraph::op::TemporaryReplaceOutputType(data1, element::f32).get());
            } else {
