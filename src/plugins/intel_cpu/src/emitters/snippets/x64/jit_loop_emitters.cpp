...
void jit_loop_begin_emitter::validate_arguments(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    ...
    OV_CPU_JIT_EMITTER_ASSERT((!need_runtime_args) || (loop_end_label != nullptr && loop_begin_label != nullptr), "has not inited labels!");
    OV_CPU_JIT_EMITTER_ASSERT(!need_runtime_args || !evaluate_once, "with dynamic work_amount cannot evaluate once!");
}
...
#define INIT_INCREMENT_REG(cond, type)                                                          \
    if (cond) {                                                                                 \
        Reg64 reg_runtime_params = abi_param1; /* defined by jit_kernel_emitter */              \
    ...
    if (!evaluate_once) {
        INIT_INCREMENT_REG(need_runtime_args_for_ptr_inc, m_ptr_increments)
        for (size_t idx = 0; idx < data_ptr_reg_idxs.size(); idx++) {
            ...
            if (ov::snippets::utils::is_dynamic_value(ptr_increments[idx])) {
                h->add(data_reg, h->ptr[reg_increments + idx * sizeof(int64_t)]);
            } else {
    ...
    INIT_INCREMENT_REG(need_runtime_args_for_final_offs, m_finalization_offsets)
    for (size_t idx = 0; idx < data_ptr_reg_idxs.size(); idx++) {
        if (!is_incremented[idx] || finalization_offsets[idx] == 0)
            continue;
        Reg64 data_reg = Reg64(static_cast<int>(data_ptr_reg_idxs[idx]));
        if (ov::snippets::utils::is_dynamic_value(finalization_offsets[idx])) {
            h->add(data_reg, h->ptr[reg_increments + idx * sizeof(int64_t)]);
        } else {
