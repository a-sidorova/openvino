...
void intel_cpu::BrgemmCopyB::validate_and_infer_types() {
    ...
    set_output_type(0, element_type, ov::PartialShape{rnd_up(K, brgemmVNNIFactor), rnd_up(N, N_blk)});
    if (m_with_comp) {
        set_output_type(1, ov::element::f32, ov::PartialShape{rnd_up(N, N_blk)});
    }
}
...
std::shared_ptr<Node> intel_cpu::BrgemmCopyB::clone_with_new_inputs(const OutputVector& new_args) const {
    ...
    return std::make_shared<BrgemmCopyB>(new_args.at(0), m_src_type, m_with_comp,
                                           get_input_port_descriptor(0).m_offset,
                                           get_output_port_descriptor(0).m_offset,
                                           m_with_comp ? get_output_port_descriptor(1).m_offset : 0);
}
