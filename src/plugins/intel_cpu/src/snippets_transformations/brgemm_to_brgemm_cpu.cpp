...
pass::BrgemmToBrgemmCPU::BrgemmToBrgemmCPU() {
    ...
    auto callback = [=](ngraph::pattern::Matcher& m) {
        ...
        const bool isAMXSupported = dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx512_core_bf16_amx_int8) ||
                                    dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx512_core_bf16_amx_bf16);
        const bool with_amx = isAMXSupported && element_type_a != ov::element::f32 && (K % brgemmVNNIFactor == 0) && (N % brgemmVNNIFactor == 0);
        // TODO: Only i8? What's about u8?
        const bool with_comp = element_type_a == ov::element::i8 && !with_amx;

        const auto offset_a = brgemm->get_input_port_descriptor(0).m_offset;
        const auto offset_b = brgemm->get_input_port_descriptor(1).m_offset;
        const auto offset_c = brgemm->get_output_port_descriptor(0).m_offset;
        ...
        if (one_of(element_type_a, ov::element::f32)) {
            ...
            const auto scratch = with_amx ? std::make_shared<ngraph::snippets::op::Buffer>(ov::Shape{4 * 1024}, ov::element::i32) :
                                 with_comp ? std::make_shared<ngraph::snippets::op::Buffer>(brgemmRepackIn1->output(1)) :
                                         nullptr;
