 ...
 * @param m_allocation_size - memory size for allocation in u8 data type. SIZE_MAX means undefined size.
...
public:
    enum { UNDEFINED_VALUE = std::numeric_limits<size_t>::max()};
    OPENVINO_OP("Buffer", "SnippetsOpset");
    Buffer() = default;
    Buffer(const OutputVector& arguments, size_t allocation_size = UNDEFINED_VALUE, size_t reg_group = UNDEFINED_VALUE,
           ov::element::Type element_type = ov::element::u8);
    ...
    bool is_allocation_size_defined() const;
    bool is_reg_group_defined() const;

protected:
    size_t m_allocation_size = UNDEFINED_VALUE;
    size_t m_reg_group = UNDEFINED_VALUE;
...
public:
    ...
    IntermediateMemoryBuffer(const ov::Output<ov::Node>& arg, size_t allocation_size = UNDEFINED_VALUE, size_t reg_group = UNDEFINED_VALUE);
...
public:
    ...
    NewMemoryBuffer(const ov::Shape& shape, size_t reg_group = UNDEFINED_VALUE, ov::element::Type element_type = ov::element::u8);
