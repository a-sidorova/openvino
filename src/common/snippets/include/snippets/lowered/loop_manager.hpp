...
public:
     ...
     * @return vector out outer loop IDs
     */
     ...
     * @param increment increment of the loop
    ...
    /**
     * @brief Find iterators of first entry loop point and last exit point (bounds of Loop) when explicit Loop exprs haven't been inserted in LinearIR
     *        Note: This method cannot be called when explicit loop exprs are inserted since Loop bounds are defined by LoopBegin and LoopEnd after the insertion!
     * @param linear_ir linear IR
     * @param loop_id target Loop ID
     * @param loop_begin_pos the ref of first expression iterator of the Loop
     * @param loop_end_pos the ref of next iterator after the last expression
     */
    void get_loop_bounds(const LinearIR& linear_ir, size_t loop_id, LinearIR::constExprIt& loop_begin_pos, LinearIR::constExprIt& loop_end_pos) const;
    /**
     * @brief Find iterators of first entry loop point and last exit point (bounds of Loop) when explicit Loop exprs haven't been inserted in LinearIR
     *        Note: This method cannot be called when explicit loop exprs are inserted since Loop bounds are defined by LoopBegin and LoopEnd after the insertion!
     * @param linear_ir linear IR
     ...
     * @param loop_end_pos the ref of next iterator after the last expression
     */
    static void get_loop_bounds(const LinearIR& linear_ir, size_t loop_id, const std::vector<LoopPort>& entries, const std::vector<LoopPort>& exits,
                                LinearIR::constExprIt& loop_begin_pos, LinearIR::constExprIt& loop_end_pos);
    /**
     * @brief Finds iterators of LoopBegin and LoopEnd when explicit Loop exprs have been already inserted in LinearIR
     * @param linear_ir linear IR
     * @param loop_id target Loop ID
     * @param loop_begin_pos the ref of first expression iterator of the Loop
     * @param loop_end_pos the ref of next iterator after the last expression
     */
    void get_explicit_loop_bounds(const LinearIR& linear_ir, size_t loop_id, LinearIR::constExprIt& loop_begin_pos, LinearIR::constExprIt& loop_end_pos) const;
