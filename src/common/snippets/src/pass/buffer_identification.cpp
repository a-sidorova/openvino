...
using BufferSet = std::vector<std::shared_ptr<op::IntermediateBuffer>>;
...
auto create_adjacency_matrix(const BufferSet& buffers) -> std::vector<bool> {
    ...
    auto update_adj_matrix = [&](const std::shared_ptr<op::IntermediateBuffer>& buffer, size_t buffer_index,
                                 const std::shared_ptr<op::IntermediateBuffer>& neighbour_buffer) {
        if (neighbour_buffer) {
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        ...
        while (const auto loop_end = ov::as_type_ptr<op::LoopEnd>(parent)) {
            ...
            for (const auto input_value : loop_begin->input_values()) {
                ...
                if (const auto neighbour_buffer = ov::as_type_ptr<op::IntermediateBuffer>(loop_in)) {
            ...
            for (const auto output : loop_end->outputs()) {
                ...
                if (buffer != consumer) {
                    if (const auto neighbour_buffer = ov::as_type_ptr<op::IntermediateBuffer>(consumer)) {
                        update_adj_matrix(buffer, i, neighbour_buffer);
                    }
                }
            }
...
bool BufferIdentification::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    // Notes: We identify only IntermediateBuffer because Allocation Buffers are used only in Brgemm case
    //        so these Buffers are always IntermediateBuffer nonadjacent
    BufferSet buffers;

    const auto ops = model->get_ordered_ops();
    for (const auto op : ops) {
        if (const auto buffer = ov::as_type_ptr<op::IntermediateBuffer>(op)) {
    ...
    for (const auto& [color, united_buffers] : color_groups) {
