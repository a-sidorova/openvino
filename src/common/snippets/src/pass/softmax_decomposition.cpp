...
ngraph::snippets::pass::SoftmaxDecomposition::SoftmaxDecomposition(const size_t vector_size, const int32_t buffer_allocation_rank) {
    ...
    auto callback = [=](ngraph::pattern::Matcher &m) {
        ...
        const auto constant_shape_exp = op::IntermediateBuffer::create_shape_constant(loop_sum_end->output(0).get_partial_shape(), buffer_allocation_rank);
        const auto buffer_exp = constant_shape_exp ? std::make_shared<op::IntermediateBuffer>(loop_sum_end->output(0), constant_shape_exp) :
                            std::make_shared<op::IntermediateBuffer>(loop_sum_end->output(0));
