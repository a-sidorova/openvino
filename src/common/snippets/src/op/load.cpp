...
std::shared_ptr<Node> Load::clone_with_new_inputs(const OutputVector& new_args) const {
    ...
    return std::make_shared<Load>(
            new_args.at(0), get_input_port_descriptor(0).m_count, get_input_port_descriptor(0).m_offset);
}
...
std::shared_ptr<Node> snippets::op::LoadReshape::clone_with_new_inputs(const OutputVector& new_args) const {
    ...
    check_new_args_count(this, new_args);
    return std::make_shared<LoadReshape>(
            new_args.at(0), get_input_port_descriptor(0).m_count, get_input_port_descriptor(0).m_offset, m_order);
}
