...
std::shared_ptr<Subgraph> Subgraph::clone() const {
    ...
    if (m_generation_linear_ir)
        result->m_generation_linear_ir = m_linear_ir->clone();
...
void Subgraph::pre_generation_transformations() const {
    ...
    OPENVINO_ASSERT(m_generation_linear_ir, "Lowered LinearIR has not been inited for pre generation transformations!");
    ...
    pipeline.run(*m_generation_linear_ir);

    m_generator->get_target_machine()->get_runtime_configurator()->init(m_generation_linear_ir);
}
...
void Subgraph::lower(const std::shared_ptr<lowered::pass::PassConfig>& lowered_pass_config,
    ...
    m_generation_linear_ir = m_linear_ir->clone(false);
...
snippets::Schedule Subgraph::generate(const BlockedShapeVector& blocked_input_shapes,
    ...
    lower(lowered_pass_config, lowered_backend_passes, min_parallel_work_amount, min_kernel_work_amount, factory);
...
snippets::Schedule Subgraph::generate_from_linear_ir(const void* compile_params) const {
    ...
    OPENVINO_ASSERT(m_generation_linear_ir, "Attempt to call generate, when linear IR was not initialized");
    auto linear_ir = *m_generation_linear_ir->clone();
...
const std::shared_ptr<RuntimeConfig>& Subgraph::update_runtime_config() const {
    ...
    OPENVINO_ASSERT(m_generation_linear_ir, "LoweredLinearIR has not been inited!");
    return m_generator->update_runtime_config(m_generation_linear_ir);
}
