...
void snippets::op::Subgraph::init_config() {
    const auto update = [](bool& value, const bool status) { value = value || status; };
    const auto ops = body_ptr()->get_ops();
    for (const auto& op : ops) {
        update(config.m_is_quantized, ov::is_type<ov::op::v0::FakeQuantize>(op));
        update(config.m_has_type_relaxed_ops, !!std::dynamic_pointer_cast<ov::op::TypeRelaxedBase>(op));
    ...
    for (const auto& op : ops) {
        if (const auto buffer = ov::as_type_ptr<ngraph::snippets::op::Buffer>(op)) {
            ...
            if (buffer->get_input_size() > 0) {
                ...
                // so after them, we should allocate new memory. Other operations (Eltwises, Convert) can be executed inplace inside Loop only at the moment.
...
snippets::Schedule snippets::op::Subgraph::generate(
    ...
    ov::pass::Manager buffer_m;
    buffer_m.register_pass<snippets::pass::BufferIdentification>();
    buffer_m.register_pass<snippets::pass::ResetBufferState>();
    buffer_m.run_passes(body_ptr());
