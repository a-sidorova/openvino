...
bool InsertLoadStore::insert_load(LinearIR& linear_ir, const LinearIR::constExprIt& data_expr_it) {
    ...
    const auto& output_td = data_expr->get_output_tensor(0);
    ...
    for (const auto& consumer_input : consumer_inputs) {
        ...
        PortManager::set_port_descriptor_ptr(load->output(0), std::make_shared<PortDescriptor>(output_td->get_tensor(),
        ...
        linear_ir.replace_input(consumer_expr, port, load_expr->get_output_tensor(0));
...
bool InsertLoadStore::insert_store(LinearIR& linear_ir, const LinearIR::constExprIt& data_expr_it) {
    const auto& loop_manager = linear_ir.get_loop_manager();
    const auto& data_expr = *data_expr_it;
    const auto& input_td = data_expr->get_input_tensor(0);
    ...
    PortManager::set_port_descriptor_ptr(store->output(0), std::make_shared<PortDescriptor>(input_td->get_tensor(),
                                                                                            input_td->get_subtensor(),
                                                                                            input_td->get_layout()));
    const auto store_expr = linear_ir.create_expression(store, {input_td});
    ...
    linear_ir.replace_input(data_expr, 0, store_expr->get_output_tensor(0));
    // Copy Loop identifies
