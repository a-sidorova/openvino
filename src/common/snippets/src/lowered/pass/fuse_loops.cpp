...
bool FuseLoops::loop_ports_are_compatible(const LinearIR::LoopManager::LoopInfoPtr& loop_upper,
    ...
    const auto upper_exit_ports = loop_upper->exit_points;
    const auto lower_entry_ports = loop_lower->entry_points;
...
bool FuseLoops::can_be_fused(const LinearIR::LoopManager::LoopInfoPtr& loop_upper, const LinearIR::LoopManager::LoopInfoPtr& loop_lower) {
    if (!loop_ports_are_compatible(loop_upper, loop_lower))
        return false;
    const auto supported_increment = loop_upper->increment == loop_lower->increment;
    const auto supported_dim_idxs = loop_upper->dim_idx == loop_lower->dim_idx;
    return supported_increment && supported_dim_idxs;
}
