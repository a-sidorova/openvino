...
bool check_transposes(const std::vector<int64_t>& before_order, const std::vector<int64_t>& after_order, const int rank, const bool transposed_b) {
    ...
    if (transposed_b) {
        ...
        for (auto i = 0; i < rank - 2; ++i)
            needed_transpose_order_before[i] = i;
        needed_transpose_order_before[rank - 1] = rank - 2;
        needed_transpose_order_before[rank - 2] = rank - 1;

        if (before_order != needed_transpose_order_before)
            return false;

        // order must be after : N-2, ..., 1, 0, N-1
        std::vector<int64_t> needed_transpose_order_after(rank);
        for (auto i = 0; i < rank - 1; ++i)
            needed_transpose_order_after[i] = rank - 2 - i;
        needed_transpose_order_after[rank - 1] = rank - 1;

        if (after_order != needed_transpose_order_after)
            return false;
    } else {
        // order must be before : N-2, N-1, 0, 1, 2, ...
        std::vector<int64_t> needed_transpose_order_before(rank);
        for (auto i = 0; i < rank - 2; ++i)
            needed_transpose_order_before[i + 2] = i;
        ...
        // order must be after : N-1, 0, 1, 2, ..., N-2
        std::vector<int64_t> needed_transpose_order_after(rank);
        for (auto i = 0; i < rank - 1 ; ++i)
            needed_transpose_order_after[i + 1] = i;
        needed_transpose_order_after[0] = rank - 1;
...
ngraph::pass::TransposeReshapeEliminationForMatmul::TransposeReshapeEliminationForMatmul() {
    ...
    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        const auto new_matmul = std::make_shared<opset1::MatMul>(input_1, input_2, transposed_a);
