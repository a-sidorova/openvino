...
void MKLDNNSnippetNode::initSupportedPrimitiveDescriptors() {
    ...
    std::vector<TensorDescCreatorTypes> tdCreatorTypes { TensorDescCreatorTypes::ncsp };
    ...
    auto range = TensorDescCreator::makeFilteredRange(creatorsMap, ndims, tdCreatorTypes);
    for (auto itr = range.first; itr != range.second; ++itr) {
        for (auto k = 0; k < inDims.size(); k++)
            config.inConfs[k].desc = itr->second->createDesc(prec, inDims[k].ToSizeVector());

        for (auto k = 0; k < outDims.size(); k++)
            config.outConfs[k].desc = itr->second->createDesc(prec, outDims[k].ToSizeVector());
